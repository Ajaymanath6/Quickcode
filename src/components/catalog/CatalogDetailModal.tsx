...
export default function CatalogDetailModal({ open, card, onClose, onSelectCard }: Props) {
  ...
  const toolbarButton =
    'inline-flex items-center gap-1.5 rounded-md px-2 py-1.5 text-xs text-brandcolor-textstrong hover:bg-brandcolor-neutralhover disabled:opacity-50'

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-brandcolor-textstrong/45 p-4"
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        className="relative flex h-[min(70vh,760px)] w-[min(60vw,1040px)] min-w-[680px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-brandcolor-strokeweak bg-white shadow-lg"
      >
        <header className="flex shrink-0 items-start gap-4 border-b border-brandcolor-strokeweak px-5 py-3.5">
        ...
        {editing ? (
          <div className="grid shrink-0 gap-3 border-b border-brandcolor-strokeweak bg-brandcolor-fill p-4 sm:grid-cols-2">
            <label className="text-xs font-medium">
              Display name
              <input
                value={name}
                onChange={(event) => setName(event.target.value)}
                className="mt-1 w-full rounded-md border border-brandcolor-strokeweak bg-white px-3 py-2 font-normal"
              />
            </label>
            <label className="text-xs font-medium">
              Description
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                className="rounded-md bg-brandcolor-primary px-3 py-1.5 text-xs text-white disabled:opacity-50"
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-brandcolor-fill p-4">
          {panel === 'image' ? (
            <div className="h-full min-h-64 overflow-auto rounded-xl border border-brandcolor-strokeweak bg-white">
          ...
          {panel === 'code' ? (
            <div className="relative h-full">
              ...
              <pre className="h-full min-h-64 whitespace-pre-wrap break-words rounded-xl border border-brandcolor-strokeweak bg-white p-4 pr-28 text-xs text-brandcolor-textstrong">
        ...
        <footer className="flex shrink-0 items-center gap-2 border-t border-brandcolor-strokeweak px-4 py-2.5">
          ...
          <span className="text-[11px] text-brandcolor-textweak">
            Use the exact published name in “A2UI: Generate Component from Prompt”.
          </span>
          <span className="ml-auto text-[11px] text-brandcolor-textweak">
            Version: {card.entry.publishedAt ? 'current publish' : 'local'} · History unavailable
          </span>
        </footer>
        {relatedLayouts.length > 0 ? (
          <div className="flex shrink-0 items-center gap-2 border-t border-brandcolor-strokeweak px-4 py-2 text-xs">
            <span className="text-brandcolor-textweak">Used in layouts:</span>
            {relatedLayouts.map((layout) => (
              <button
                key={layout.entry.componentId}
                className="text-brandcolor-textstrong underline"
        ...
        {notice ? (
          <div role="status" className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-md bg-brandcolor-textstrong px-3 py-2 text-xs text-white shadow-lg">
            {notice}
          </div>
        ) : null}
      </div>
